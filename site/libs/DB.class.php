...
class DB 
{
	...
	static private $_transtatus = 0;
	static private $_chknested  = 0;	//检查是否事务嵌套
	...
	static function Instance($config = array()) 
	{
		...
		if(!isset(self::$mInstance[$key]) || empty(self::$mInstance[$key])) { 
	...
	function __construct($config = array()) {
		...
		try {
			...
			//self::retryDBConnect($config);    //ison.zhang
		}
	}
	
	//事务  在tranBegin()与tranEnd()之间插入常规语句就可以
	static public function tranBegin($nested = '')
	{
		if(!self::$_chknested){			//解决嵌套事务
			if(!$nested){
				self::$_chknested = 1;
			}else{
				self::$_chknested = $nested;
			}
			
			self::$_transaction = true;
			self::$_transtatus = 1;
			self::Instance();
			self::$mConnection->beginTransaction();
		}
	}
	static public function tranEnd($nested = '')
	{
		if(!$nested) $nested = 1;
		if(self::$_chknested == $nested){
			$status = 0;
			if(!self::$_transtatus){
	...
	static public function tranStop()
	{
		self::$_transtatus = 0;
	}
	 ...
	 * 查询单条数据
	 */	
	static function getRow(array $where, $tableName, $select = '*', $order='')
	{
		if(!$where || !$tableName) return false;
		$options['one'] = 1;
		$options['select'] = $select;
		$options['condition'] = $where;
		if($order) $options['order'] = "ORDER BY $order";
	...
	*/
	...
	static function Execute($sql, array $params = array(), $retry = 0) {
		...
		try {
			...
			if(!is_object($sth)) throw new Exception('Error: bad sql');
